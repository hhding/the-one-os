...
int32_t sys_open(const char* pathname, uint8_t flag) {
    ...
    // printk("call file_open on %s..\n", pathname);
    fd = file_open(inode_no, flag);
    // printk("file_open ret..\n");

sys_open_ok:
    // printk("call dir_close on ok..\n");
    dir_close(search_record.parent_dir);
    // printk("call dir_close for %s on ok done. ret: fd: %d.\n", pathname, fd);
    return fd;

sys_open_err:
    // printk("call dir_close on err..\n");
    dir_close(search_record.parent_dir);
    // printk("call dir_close for %s on err done..\n", pathname);
    return -1;    
}
